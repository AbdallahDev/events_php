...
<html>
    <head>
        <title>الجلسات الحالية والمستقبلية</title>
        <!--header file inclusion-->
        <?php include_once 'include/header.php'; ?>
        <script src="../js/jquery-3.2.1.min.js" type="text/javascript"></script>
        <script>
            function show_sessions() {
                $.get('sessions_get_current_future.php', function (data, status) {
                    $('#sessions').html(data);
                });
            }
        ...
        <div>
            ...
            <div>
                <nav class="w3-sidebar w3-bar-block w3-white w3-card-2 w3-animate-left w3-xxlarge" style="display:none;z-index:2" id="mySidebar">
                    ...
                    <?php
                    //here make the sidemenu for the users from 
                    //the legislative affairs directorate 
                    if ($_SESSION['directorate'] == 2) {
                        if ($_SESSION['user_type'] == 1) {
                            //here make the menu for the admin
                            ?>
                            <a class="w3-bar-item w3-button" href='sessions_past.php'>الأرشيف</a>
                            <?php
                        } else {
